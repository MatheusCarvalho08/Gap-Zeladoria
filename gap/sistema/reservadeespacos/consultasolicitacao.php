...
$sql = "SELECT * FROM solicitacaohome ORDER BY id DESC";
...
session_start();
  ...
  if(isset($_POST['submit'])){
    ...
    $result = mysqli_query($conexao, "INSERT INTO solicitacaohome(nome,sobrenome,documento,bloco,telefone,horario_disp,horario,data_reserva) VALUES ('$nome','$sobrenome','$documento','$bloco','$telefone','$horario_disp','$horario','$data_reserva')");
}

  if(!empty($_GET['search']))
  {
      $data = $_GET['search'];
      $sql = "SELECT * FROM solicitacaohome WHERE id LIKE '%$data%' or nome LIKE '%$data%' or documento LIKE '%$data%' or telefone LIKE '%$data%' ORDER BY id DESC";
  }
  else
  {
      $sql = "SELECT * FROM solicitacaohome ORDER BY id DESC";
    ...
    <header>
        <nav class="navigation">
            ...
            <ul class="nav-menu">
                ...
                <li class="nav-item"><a href="/gap/sistema/reservadeespacos/hometheater.php">Solicitação de Uso</a></li>
            </ul>
            <div class="sair-menu">
...
<main class="main-container">
        <center><div class="main-title">
          <h2>PÁGINA DE SOLICITAÇÃO DE USO</h2>
        </div></center>
        ...
        <table class="table text-white table-bg">
              <thead>
                <tr>
                  ...
                  <th scope="col">Horário de uso</th>
              ...
              <tbody>
                  <?php 
                    while($user_data = mysqli_fetch_assoc($result))
                    {
                        ...
                        echo "<td>".$user_data['horario_disp']."</td>";
                        echo "<td>".$user_data['horario']."</td>";
                        echo "<td>".$user_data['data_reserva']."</td>";
                        echo "<td>
                        <a class='btn btn-sm btn-danger' href='deletes/deletehome.php?id=$user_data[id]' title='Deletar'>
...
<script>
        ...
        function searchData()
        {
            window.location = 'consultasolicitacao.php?search='+search.value;
        }
</script>
