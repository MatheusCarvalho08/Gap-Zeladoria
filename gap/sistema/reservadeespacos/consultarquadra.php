...
$sql = "SELECT * FROM reservaquadra ORDER BY id DESC";
...
session_start();
  ...
  if(isset($_POST['submit'])){
    ...
    $result = mysqli_query($conexao, "INSERT INTO reservaquadra(nome,sobrenome,cpf,telefone,bloco,horario,data_reserva) VALUES ('$nome','$sobrenome','$cpf','$telefone','$bloco','$horario','$data_reserva')");
}

  if(!empty($_GET['search']))
  {
      $data = $_GET['search'];
      $sql = "SELECT * FROM reservaquadra WHERE id LIKE '%$data%' or nome LIKE '%$data%' or cpf LIKE '%$data%' or telefone LIKE '%$data%' ORDER BY id DESC";
  }
  else
  {
      $sql = "SELECT * FROM reservaquadra ORDER BY id DESC";
    ...
    <header>
        <nav class="navigation">
            ...
            <ul class="nav-menu">
                ...
                <li class="nav-item"><a href="/gap/sistema/reservadeespacos/quadra.php">Reserva da Quadra</a></li>
            </ul>
            <div class="sair-menu">
...
<main class="main-container">
        <center><div class="main-title">
          <h2>PÁGINA DE CONSULTA DAS RESERVAS DA QUADRA</h2>
        </div></center>
              ...
              <tbody>
                  <?php 
                    while($user_data = mysqli_fetch_assoc($result))
                    {
                        ...
                        echo "<td>
                        <a class='btn btn-sm btn-danger' href='deletes/deletequadra.php?id=$user_data[id]' title='Deletar'>
...
<script>
        ...
        function searchData()
        {
            window.location = 'consultarquadra.php?search='+search.value;
        }
</script>
